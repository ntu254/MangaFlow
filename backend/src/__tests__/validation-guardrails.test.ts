...
function submitTask(accessToken: string, taskId: string, idempotencyKey: string) {
  return request(createApp())
    ...
    .send({
      ...
      notes: "Submitted from canonical task command",
      fileKey: `${idempotencyKey}.png`,
    });
}
...
describe("MF-022 Backend Validation & Mass Assignment Guardrails", () => {
  ...
  it("rejects PATCH /series/:id with protected fields", async () => {
    ...
    // Series creation now requires a Board-approved proposal; insert a fixture
    // directly since this test only exercises PATCH guardrails.
    const seriesId = "s-mf022-protected";
    await SeriesModel.create({
      id: seriesId,
      slug: seriesId,
      title: "MF-022 test series",
      authorId: "u-mangaka",
  ...
  it("rejects PATCH /series/:id with invalid field type", async () => {
    const editor = await loginAs("tanaka@beachread.jp");
    const seriesId = "s-mf022-type";
    await SeriesModel.create({
      id: seriesId,
      slug: seriesId,
      title: "MF-022 type test",
      authorId: "u-mangaka",
      authorName: "Inoue Takehiko",
      editorId: "u-editor",
      editorName: "Tanaka Akira",
  ...
  it("RESUBMIT persists the revised proposal, real files, and resolved checklist", async () => {
    ...
    expect(response.body.data.status).toBe("PENDING_EDITOR");
    ...
    const editor = await loginAs("tanaka@beachread.jp");
    await request(createApp())
      .post("/api/proposals/p-full-revision-test/actions/CLAIM")
      .set("Authorization", `Bearer ${editor.accessToken}`)
      .send({})
      .expect(200);
  ...
  it("rejects editor decisions until the editor has claimed the proposal", async () => {
    const editor = await loginAs("tanaka@beachread.jp");
    await ProposalModel.create({
      id: "p-unclaimed-decision-test",
      slug: "unclaimed-decision-test",
      title: "Unclaimed decision",
      authorId: "u-mangaka",
      authorName: "Mangaka",
      status: "PENDING_EDITOR",
    });

    for (const action of ["FORWARD", "REQUEST_CHANGES", "REJECT"]) {
      const response = await request(createApp())
        .post(`/api/proposals/p-unclaimed-decision-test/actions/${action}`)
        .set("Authorization", `Bearer ${editor.accessToken}`)
        .send({ comment: "A sufficiently detailed editorial reason." })
        .expect(409);
      expect(response.body.code).toBe("REVIEW_CLAIM_REQUIRED");
    }
  });

  it("persists the claimed editor checklist and requires 6/6 before Board forwarding", async () => {
    const editor = await loginAs("tanaka@beachread.jp");
    await ProposalModel.create({
    ...
    const incomplete = await request(createApp())
      .post("/api/proposals/p-claim-forward-test/actions/FORWARD")
      .set("Authorization", `Bearer ${editor.accessToken}`)
      .send({})
      .expect(409);
    expect(incomplete.body.code).toBe("EDITORIAL_CHECKLIST_INCOMPLETE");
    ...
    const saved = await request(createApp())
      .post("/api/proposals/p-claim-forward-test/actions/UPDATE_EDITORIAL_CHECKLIST")
      .set("Authorization", `Bearer ${editor.accessToken}`)
      .send({ editorialChecklist: checklist })
      .expect(200);
    expect(saved.body.data.editorialChecklist).toMatchObject(checklist);
    expect(saved.body.data.editorialChecklist.completedById).toBe(editor.user.id);
  ...
  it("rejects task assignment when the selected page has no durable source asset", async () => {
    const mangaka = await loginAs("inoue@beachread.jp");
    const chapterId = "ch-s-berserk-prod-4";
    ...
    const response = await request(createApp())
      ...
      .send({
        ...
        chapterId,
        pageId: page.id,
        title: "Task without source page",
        type: "background",
        assigneeId: "u-assist",
        rateCode: rate.code,
        quantity: 1,
        ...
        priority: "normal",
        instructions: "Clean the background.",
      })
      .expect(409);
    expect(response.body.code).toBe("PAGE_SOURCE_REQUIRED");
  });
...
describe("MF-027 Assistant Task Studio Live Submission Flow", () => {
  ...
  it("ASSISTANT can create a submission via POST /api/tasks/:taskId/submit", async () => {
  ...
  it("ASSISTANT can create a submitted work item visible to live review filters", async () => {
    const assistant = await loginAs("jun@beachread.jp");
    const createRes = await submitTask(
      assistant.accessToken,
      "tsk-mf027-test",
      "idem-mf027-visible",
    ).expect(201);
  ...
  it("GET /api/submissions filters by assistantId", async () => {
    ...
    await submitTask(assistant.accessToken, "tsk-mf027-test", "idem-mf027-filter").expect(201);
  ...
  it("GET /api/tasks/:taskId/submissions returns submissions for a task", async () => {
    const assistant = await loginAs("jun@beachread.jp");
    // Create a submission first
    await submitTask(assistant.accessToken, "tsk-mf027-test", "idem-mf027-task-subs").expect(201);
  ...
  it("MANGAKA can request revision on a submission", async () => {
    ...
    const revRes = await request(createApp())
      ...
      .expect(200);
    expect(revRes.body.data.status).toBe("REVISION_REQUESTED");
  });

  it("legacy submission creation endpoint is deprecated", async () => {
    const assistant = await loginAs("jun@beachread.jp");
    await request(createApp())
...
describe("MF-028 Mangaka Review Queue Live Submission Review", () => {
  ...
  it("Mangaka can see review queue (submissions with PENDING status)", async () => {
    ...
    // Insert a submission directly with canonical PENDING status.
    ...
    // Verify it appears in the PENDING filter
  ...
  it("GET /api/submissions?status=PENDING filters correctly", async () => {
    ...
    // Verify only PENDING ones appear in queue
    const queueRes = await request(createApp())
      .get("/api/submissions?status=PENDING")
      .set("Authorization", `Bearer ${mangaka.accessToken}`)
      .expect(200);
  ...
  it("Self-approval is blocked for submissions (SELF_APPROVAL_BLOCKED)", async () => {
    ...
    // Insert a PENDING submission where assistant is both creator and reviewer
