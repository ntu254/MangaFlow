...
export const listUsers = asyncRoute(async (_req: AuthedRequest, res) => {
  ...
  ok(
    res,
    users.map((user: any) => adminService.adminUser(user)),
  );
});
...
export const getUser = asyncRoute(async (req: AuthedRequest, res) => {
  ...
  if (!user) {
    return res
      .status(404)
      .json({ success: false, data: null, message: "User not found.", code: "NOT_FOUND" });
  }
...
export const resetUserPassword = asyncRoute(async (req: AuthedRequest, res) => {
  ...
  const updated = await adminService.resetUserPassword(
    req,
...
// Routes are only registered outside production (admin.routes.ts); this check is
// a defense-in-depth backstop in case the handler is ever invoked directly.
function assertDemoToolingAllowed() {
  if (process.env.NODE_ENV === "production") {
    throw new AppError(
      403,
      "Demo data tooling is disabled in production.",
