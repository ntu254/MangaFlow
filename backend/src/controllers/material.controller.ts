...
import { filterFromQuery, paginated, patchById, stripMongo } from "./helpers.js";
import { parseBody, rejectProtectedFields, sanitizePatch } from "../validators/common.js";
import { createMaterialSchema, patchMaterialSchema, addMaterialVersionSchema } from "../validators/material.schema.js";
import type { AuthedRequest } from "../types.js";

export const listMaterials = asyncRoute(async (req: AuthedRequest, res) =>
  paginated(req, res, MaterialModel, filterFromQuery(req), { updatedAt: -1 }),
);
...
export const patchMaterial = asyncRoute(async (req: AuthedRequest, res) => {
  ...
  const allowedFields = ["title", "kind", "chapterId", "type", "category", "description", "fileKey", "url", "thumbnailUrl", "mimeType", "size", "tags", "metadata"];
