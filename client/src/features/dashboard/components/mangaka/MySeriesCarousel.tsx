...
const getActionColor = (action?: string) => {
  ...
  if (a.includes("asap")) return "text-amber-600 dark:text-amber-400 font-bold hover:text-amber-700";
  if (a.includes("finalize")) return "text-purple-600 dark:text-purple-400 font-bold hover:opacity-80";
...
export function MySeriesCarousel({ mangakaId }: { mangakaId: string }) {
  ...
  const handleActionClick = (e: React.MouseEvent, s: any) => {
    ...
    else if (s.nextAction?.includes('Upload')) navigate({ to: `/app/series/${s.id}/chapters/current/pages/upload` });
  ...
  return (
    <section className="mb-8 relative">
      ...
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
        {mine.map((s) => {
          const progressVal = s.pages && s.pages.total > 0 ? Math.round((s.pages.uploaded / s.pages.total) * 100) : 0;
          ...
          const nextChapterText = chapterNumMatch ? `Chapter ${parseInt(chapterNumMatch[0]) + 1}` : "Next chapter";
          ...
          return (
            <Link
              ...
              className={`group relative flex w-[350px] flex-none shrink-0 flex-row overflow-hidden rounded-xl border transition-all hover:-translate-y-[1px] hover:shadow-[0_8px_24px_rgba(5,24,38,0.08)] ${
                isAtRisk ? "border-destructive/80 bg-card hover:border-destructive" : "border-foreground/10 bg-card hover:border-foreground/20"
              }`}
            >
              <div className="w-[125px] shrink-0 bg-foreground/5">
                <img src={s.cover} alt={s.title} className="h-full w-full object-cover transition group-hover:scale-[1.02]" />
              </div>
              
              <div className="flex flex-1 flex-col p-4 relative">
                <div className="min-w-0 mb-3">
                  <div className="truncate text-[14px] font-bold leading-tight text-foreground">{s.title}</div>
                ...
                <div className="mt-auto flex flex-col text-[11px] text-foreground/70">
                  ...
                  <div className="mb-3">
                    Pages {s.pages?.uploaded}/{s.pages?.total} &middot; Tasks <span className={getTaskColor(s.pendingTasks)}>{s.pendingTasks} {s.status === 'board-review' || s.status === 'editor-review' ? 'waiting' : 'pending'}</span>
                  </div>
      ...
      <div className="absolute -left-4 top-10 bottom-4 w-12 bg-gradient-to-r from-background to-transparent flex items-center justify-center pointer-events-none">
        <button onClick={scrollLeft} className="flex h-7 w-7 items-center justify-center rounded-full border border-foreground/15 bg-background shadow-sm text-foreground/60 hover:text-foreground pointer-events-auto">
      ...
      <div className="absolute -right-4 top-10 bottom-4 w-12 bg-gradient-to-l from-background to-transparent flex items-center justify-center pointer-events-none">
        <button onClick={scrollRight} className="flex h-7 w-7 items-center justify-center rounded-full border border-foreground/15 bg-background shadow-sm text-foreground/60 hover:text-foreground pointer-events-auto">
