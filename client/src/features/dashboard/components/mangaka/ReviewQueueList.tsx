...
export function ReviewQueueList({ data }: { data?: any }) {
  const awaitingMe = data?.pendingReviewsList || [];
  ...
  return (
    <div className="flex-1 rounded-xl border border-foreground/10 bg-card p-5 shadow-sm">
      ...
      <div className="space-y-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="truncate text-[13px] font-medium text-foreground">
                    <span className="font-bold">{seriesLabel}</span> - Chapter {chapterLabel}
                  </div>
        ...
        {awaitingMe.length === 0 && (
          <div className="text-[13px] text-foreground/50 py-4 text-center">No pending reviews.</div>
        )}
      </div>
    </div>
  );
