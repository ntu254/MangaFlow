...
import { activeSeriesData } from '../../constants/dashboard'
const getThumbnail = (name: string, id: number) => {
  const colors = ['bg-purple-100 text-purple-700', 'bg-blue-100 text-blue-700', 'bg-red-100 text-red-700', 'bg-emerald-100 text-emerald-700'];
                    ...
                    <div className="flex justify-between w-full z-10 relative px-2">
                      {[1, 2, 3, 4].map((step, idx) => {
                        ...
                        if (series.progressStage >= step) {
                          dotClass = series.progressStage === step && series.stage === 'At Risk' ? 'border-red-500 bg-white' : 'border-purple-500 bg-purple-500';
                        }
                        return (
