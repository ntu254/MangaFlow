...
import type { StudioSelection, StudioTool } from "@/entities/series/model/studio-types";
import {
...
export function StudioTab({
  ...
  const hasActiveTaskOnSelectedRegion = Boolean(
    ...
    selectedRegionLinkedTask.status !== "EDITOR_APPROVED" &&
    selectedRegionLinkedTask.status !== "REJECTED" &&
    selectedRegionLinkedTask.status !== "CANCELLED",
  );
  const canCreateTask =
    permissions.canCreateTask && Boolean(selectedRegion) && !hasActiveTaskOnSelectedRegion;
  const canSubmitSelectedTask = permissions.canSubmitTask && selectedTask?.assigneeId === user.id;
  const canRunPageAi =
