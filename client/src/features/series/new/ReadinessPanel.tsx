          ...
          <div className="flex gap-4">
            <div className="relative group h-32 w-24 shrink-0">
              {coverDrafts.length > 0 ? (
                ...
                >
                  {inflight ? <Loader2 className="h-6 w-6 mb-1 animate-spin" /> : <Upload className="h-6 w-6 mb-1 group-hover/upload:-translate-y-0.5 transition-transform" />}
                  <span className="text-[9px] font-medium">{inflight ? "Uploading..." : "Upload Cover"}</span>
                </button>
              )}
            ...
            <div className="flex flex-col min-w-0">
              <div className="mb-2">
                ...
                <div className="text-[13px] font-bold text-foreground/90 truncate">{v.title || "Untitled"}</div>
              </div>
              <div className="mb-2">
                <div className="text-[10px] uppercase tracking-wider text-foreground/50">Genre</div>
                <div className="text-[12px] text-foreground/80 truncate">{v.genres?.join(", ") || "None"}</div>
              </div>
              <div className="mb-2">
                <div className="text-[10px] uppercase tracking-wider text-foreground/50">Audience</div>
                <div className="text-[12px] text-foreground/80">{v.targetAudience || "None"}</div>
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-wider text-foreground/50">Publication Type (Proposed)</div>
                <div className="text-[12px] text-foreground/80">{v.preferredCadence === "WEEKLY" ? "Weekly" : v.preferredCadence === "MONTHLY" ? "Monthly" : "No preference"}</div>
              </div>
            </div>
          </div>
          
          <div className="mt-4 border-t border-foreground/5 pt-3">
            <div className="text-[10px] uppercase tracking-wider text-foreground/50 mb-1">Logline</div>
...
function ChecklistItem({ label, hint, isDone, isOptional }: { label: string, hint?: string, isDone: boolean, isOptional: boolean }) {
  return (
    <li className="flex items-start gap-3 text-[13px]">
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex items-center justify-between">
          <span className={isDone ? "font-medium text-foreground/90" : "text-foreground/70"}>{label}</span>
          <span className={[
            "text-[10px] font-medium",
            isDone && !isOptional ? "text-emerald-500" :
            isDone && isOptional ? "text-emerald-500" :
            "text-foreground/40"
          ].join(" ")}>
            {isDone && !isOptional ? "Completed" :
             isDone && isOptional ? "Attached" :
             isOptional ? "Optional" : "Pending"}
          </span>
        </div>
        {hint && (
          <span className="mt-0.5 text-[10px] text-foreground/50">{hint}</span>
        )}
      </div>
    </li>
  );
