...
import { TrendingUp, Trophy, ArrowUpRight, ArrowDownRight, Minus, PenTool, BookOpen } from "lucide-react";
...
function ActivityPage() {
  ...
  const mangaka = findStaff(series.mangakaId);
  const editor = findStaff(series.editorId);
      ...
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[1fr_320px] mt-8">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-[#E5DFD3] bg-card p-5 shadow-sm dark:border-border">
            <h3 className="mb-4 text-[12px] font-bold uppercase tracking-wider text-muted-foreground">Key Personnel</h3>
              ...
              {history.length > 0 ? (
                <div className="flex flex-col">
                  {history.map((record, index) => {
                    ...
                    let trend = "flat";
                    if (prevRecord) {
                      if (record.rank < prevRecord.rank) trend = "up"; // smaller number is better
                    ...
                    return (
                      ...
                      >
                        <div>
                          <div className="text-[13px] font-medium text-foreground">{record.period.replace("-W", " Week ")}</div>
                          <div className="mt-0.5 flex items-center gap-1 text-[11px] font-medium">
                            {trend === "up" && <span className="flex items-center text-emerald-600"><ArrowUpRight className="h-3 w-3 mr-0.5" /> +{prevRecord.rank - record.rank}</span>}
                            {trend === "down" && <span className="flex items-center text-red-600"><ArrowDownRight className="h-3 w-3 mr-0.5" /> -{record.rank - prevRecord.rank}</span>}
                            {trend === "flat" && prevRecord && <span className="flex items-center text-muted-foreground"><Minus className="h-3 w-3 mr-0.5" /> flat</span>}
                            {trend === "flat" && !prevRecord && <span className="text-muted-foreground">No prior data</span>}
                          </div>
                        </div>
                        
                        <div className="flex items-center gap-2">
                          {isTop3Any && (
                            <Trophy className={`h-4 w-4 ${isTop1 ? 'text-amber-500' : isTop2 ? 'text-slate-400' : 'text-amber-700'}`} />
                          )}
                          <div className={`flex h-8 w-8 items-center justify-center rounded-full font-bold text-[14px] shadow-sm
                            ${isTop1 ? 'bg-amber-100 text-amber-700 border border-amber-200' : 
                              isTop2 ? 'bg-slate-100 text-slate-700 border border-slate-200' : 
                              isTop3 ? 'bg-amber-50 text-amber-900 border border-amber-200/50' : 
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="mt-1 text-[12px] text-muted-foreground">Ranking history will appear here once published.</p>
                </div>
              )}
            </div>
          </div>
