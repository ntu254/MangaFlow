...
import { buildSidebarSections, sidebarBadgeClass, type SidebarItem } from "./role-sidebar.config"
import { useAdminSidebarSummary } from "./useAdminSidebarSummary"
...
export function RoleSidebar({ isOpen, onClose, collapsed, onToggleCollapse }: RoleSidebarProps) {
  ...
  const roleLabel = user?.role
    ...
    : "MangaFlow Studio"
  const summary = useAdminSidebarSummary(roleName)
  const sections = useMemo(
    () => buildSidebarSections(roleName, user?.role, summary),
    [roleName, summary, user?.role],
  )
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 space-y-sm overflow-y-auto px-sm py-sm">
                ...
                {section.items.map((item: SidebarItem) => {
                  ...
                  return (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          ...
                          collapsed ? "mx-auto w-11 justify-center px-0" : "mx-md px-md",
                          isActive
                    ...
                    >
                      ...
                      {!collapsed && (
                        <>
                          <span className="min-w-0 flex-1 truncate text-label-md">{item.label}</span>
                          {item.badge ? (
                            <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold", sidebarBadgeClass(item.badgeTone))}>
                              {item.badge}
                            </span>
                          ) : null}
                        </>
