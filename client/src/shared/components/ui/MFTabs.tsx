...
export const MFTabs = forwardRef<HTMLDivElement, MFTabsProps>(
  ({ className, tabs, activeTab, onTabChange, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("flex gap-1 rounded-full bg-surface-container p-1", className)} {...props}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={cn(
              "inline-flex items-center gap-2 rounded-full px-4 py-2 text-label-sm font-semibold transition-all duration-150",
