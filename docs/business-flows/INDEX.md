...
The complete team/Tantou contract is documented in [15 - Team and Tantou](15-team-and-tantou.md).

| API Role    | Web Role    | Description                                                                                                                                                                                                    |
| ----------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ADMIN`     | `admin`     | User account lifecycle and Board Chair designation management; also owns managed notifications, read-only dashboards, RateTable, and dev-only demo data. No editorial/production/governance workflow authority |
...
Special flag: `isChair` (Board). Tantou is an active `EDITOR` membership on a Series, not a role or designation.
...
5. ✅ One Page has at most one active Page Task (`studio_task_one_active_page_assignment`).
...
15. ✅ Supporting Materials are optional status-free attachments. Only the owning
    Mangaka mutates them; they never gate Chapter, Tantou, publication, or Board flows.
16. ✅ Admin cannot execute editorial or production workflow actions (rankings
    ...
    account lifecycle, Board Chair designation, managed notifications, read-only
...
17. ✅ At most five Board seats are active; exactly one active Board Chair and
    one active Board Chair may exist. Designation reassignment is transactional.
...
> docs. See the separated _Documentation consistency_ vs _Current implementation
> compliance_ matrices in
...
| TECH-FINDING-03 | **Superseded.** Regions are annotation-only; new work uses one active Page Task and the page-task migration reports legacy conflicts | `models.ts`; `migrate-page-task-contract.ts` | [14](14-regions.md) |
...
redundant Chapter `ARCHIVE` transition has been removed; Chapters now follow the
...
| - RELEASE_CLAIM            | (action)                                     | `workflow.service.ts`        | The claiming EDITOR only                    |
...
| - VOTE                     | (action)                                     | `workflow.service.ts`        | BOARD in an active VotingSession            |
...
| - ARCHIVE                  | (action)                                     | `workflow.service.ts`        | Owning MANGAKA; requires non-empty `reason` |
...
| Add member       | Retired (`410 INVITE_ACCEPTANCE_REQUIRED`)       | `series.controller.ts`     | No direct membership creation |
...
| Patch regions (bulk) | Retired (`410 ENDPOINT_DEPRECATED`) | `studio.controller.ts` | — |
...
| Patch tasks (bulk)      | Retired (`410 ENDPOINT_DEPRECATED`)                      | `studio.controller.ts`       | —                                                        |
...
| Task action             | `POST /api/studio/tasks/:taskId/actions/:action`          | `studio.controller.ts:398`   | MANGAKA, assigned ASSISTANT                              |
| - ACCEPT / REJECT       | (action)                                                  | `task-submission.service.ts` | Assigned ASSISTANT; decision required before work starts |
| - START                 | (action)                                                  | `workflow.service.ts:2105`   | Accepted assigned ASSISTANT                              |
...
New task creation is Page-level: `pageId` is required, `regionId` is rejected,
and the unique active-page invariant prevents two assistants from working the
...
Material has no status or review workflow. Legacy status fields and archived
records are handled by the dry-run-first `migrate:material-attachments` scripts.
...
Notification state is intentionally limited to unread/read. Legacy archived
notifications are handled by the dry-run-first
`migrate:notification-read-model` migration.
...
| Assign series editor | `POST /api/series/:seriesId/editor`   | `tantou.controller.ts`   | Owning MANGAKA; target must be active EDITOR |
| Remove series editor | `DELETE /api/series/:seriesId/editor` | `tantou.controller.ts`   | Owning MANGAKA; no open workload             |
...
_Generated from backend route registrations (`routes/*.ts`), controller handlers (`controllers/*.ts`), and workflow service logic (`services/workflow.service.ts`)._
