...
export function useChapterReviewsQuery(chapterId: string) {
  return useQuery<ChapterReview[], Error>({
    queryKey: chapterKeys.reviews(chapterId),
    queryFn: () => apiRequest<ChapterReview[]>(`/chapters/${chapterId}/reviews`),
    enabled: !!chapterId,
    staleTime: 30000,
  });
}
...
export function useChapterActionMutation(chapterId: string, seriesId?: string) {
  ...
  return useMutation<Chapter, Error, { action: string; payload?: unknown }>({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: chapterKeys.reviews(chapterId) });
      if (seriesId) {
