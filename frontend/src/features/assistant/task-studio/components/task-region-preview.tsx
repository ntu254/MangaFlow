...
export function TaskRegionPreview({
  ...
  return (
    <div className="relative grid h-full place-items-center overflow-hidden rounded-md border border-border bg-muted/30 p-4">
      ...
      >
        {page.fileKey ||
        isRenderableFileUrl(page.fileUrl ?? page.imageUrl) ||
        isRenderableFileUrl(coverFallback) ? (
          <ResolvedImage
            fileKey={page.fileKey}
            fallbackUrl={
              (isRenderableFileUrl(page.fileUrl ?? page.imageUrl)
                ? page.fileUrl || page.imageUrl
                : undefined) ?? (isRenderableFileUrl(coverFallback) ? coverFallback : undefined)
            }
            alt=""
          ...
          <div className="absolute inset-0 grid place-items-center rounded bg-background text-xs text-muted-foreground">
            <span className="max-w-[240px] text-center">
              Source page unavailable. Ask the Mangaka to upload it before starting this task.
            </span>
          </div>
        )}
        {region ? (
