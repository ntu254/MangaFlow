...
export function AssistantTaskTable({
  ...
  const { sorted, sortKey, sortDirection, toggleSort } = useSortableData(tasks, {
    title: (t) => t.title,
    dueAt: (t) => (t.dueAt ? new Date(t.dueAt) : undefined),
    priority: (t) => PRIORITY_ORDER[t.priority] ?? 0,
    status: (t) => getVisualTaskStatus(t),
  });
