...
export function EditorDashboard() {
  ...
  return (
    <PageShell dashboardRole="editor">
      <PageHeader
        eyebrow="Editor workspace"
        title={`Today, ${user.name.split(" ")[1] ?? user.name}`}
        actions={
          <Link
        ...
        description={`${allItems.length} items to review (${counts.submission} submissions live) · ${counts.deadlineRisk} series at deadline risk · ${counts.ready} chapters ready to publish`}
      />
      ...
      <SummaryGrid className="grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        <StatCard
      ...
      >
        {focus.length === 0 ? (
          <p className="text-xs text-muted-foreground">No items to review</p>
        ) : (
      ...
      >
        {deadlineRows.length === 0 ? (
          <p className="text-xs text-muted-foreground">You have not been assigned any series</p>
        ) : (
      ...
      <PageSection
        ...
        description="Latest editorial notifications and updates."
      >
        {myNotifs.length === 0 ? (
