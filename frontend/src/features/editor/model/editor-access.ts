...
export function buildReviewQueue(
  ...
  for (const ch of chapters) {
    ...
    const blocking = comments.filter(
      (c) => c.chapterId === ch.id && c.isBlocking && c.status !== "RESOLVED",
    ).length;

    if (ch.status === "TANTOU_REVIEW") {
      items.push({
        id: `ch-${ch.id}`,
        kind: "CHAPTER",
        refId: ch.id,
...
export function getPublicationReadiness(
  ...
  const blocking = comments.filter(
    (c) => c.chapterId === chapter.id && c.isBlocking && c.status !== "RESOLVED",
  ).length;
  const pagesUploaded = chapter.pages.length;
  const hasManuscript = (chapter.materials ?? []).length > 0;
  const reasons: string[] = [];
  if (pagesUploaded === 0) reasons.push("No pages uploaded");
  if (blocking > 0) reasons.push(`${blocking} blocking comments unresolved`);
  if (chapter.status !== "READY_FOR_PUBLICATION") reasons.push("Chapter not yet approved");
