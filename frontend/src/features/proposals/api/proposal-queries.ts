import type { ProposalVersion, SeriesProposal } from "@/entities/proposal/model/proposal-types";
...
export const proposalKeys = {
  ...
  versions: (proposalId: string) => ["proposals", "detail", proposalId, "versions"] as const,
  version: (proposalId: string, versionId: string) =>
    ["proposals", "detail", proposalId, "versions", versionId] as const,
...
export function useProposalVersionsQuery(proposalId: string) {
  return useQuery<ProposalVersion[]>({
    queryKey: proposalKeys.versions(proposalId),
    queryFn: () => apiRequest<ProposalVersion[]>(`/proposals/${proposalId}/versions`),
    enabled: !!proposalId,
    staleTime: 30000,
  });
}

export function useProposalVersionQuery(proposalId: string, versionId: string) {
  return useQuery<ProposalVersion>({
    queryKey: proposalKeys.version(proposalId, versionId),
    queryFn: () => apiRequest<ProposalVersion>(`/proposals/${proposalId}/versions/${versionId}`),
  ...
  return useMutation<SeriesProposal, Error, Record<string, unknown> & { id?: string }>({
    ...
    onSuccess: (_data, variables) => {
      ...
      if (activeId) {
        ...
        queryClient.invalidateQueries({ queryKey: proposalKeys.versions(activeId) });
      }
      queryClient.invalidateQueries({ queryKey: proposalKeys.all });
      if (seriesId) {
