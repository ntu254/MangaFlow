...
export function ManuscriptUploader({
  ...
  const handle = async (file: File) => {
    ...
    if (kb > MAX_KB) {
      toast.error(`File ${kb}KB exceeds ${MAX_KB / 1024}MB limit.`);
      return;
    }
    try {
        ...
        <div className="flex items-center gap-3 rounded border border-border bg-background p-3 text-xs">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-muted-foreground">
              ...
              {value.pageCount ? ` · ${value.pageCount} pages` : ""}
            </p>
          </div>
        </div>
      ) : required ? (
        <p className="text-[10px] text-rose-700">
          No manuscript uploaded — required before submission.
        </p>
      ) : null}
      <Textarea
        rows={2}
        placeholder="Manuscript notes (optional)..."
        value={value?.note ?? ""}
...
export function MaterialsUploader({
  ...
  const add = async (file: File) => {
    ...
    if (!title.trim()) {
      toast.error("Please provide a short title for the material first.");
      return;
    }
    const kb = Math.round(file.size / 1024);
    if (kb > MAX_KB) {
  ...
  return (
    <div className="space-y-2">
      ...
      {required && items.length === 0 ? (
        <p className="text-[10px] text-rose-700">Required before submission.</p>
      ) : null}
      {items.length > 0 ? (
