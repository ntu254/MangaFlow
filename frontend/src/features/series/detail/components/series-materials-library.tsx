...
import {
  toSeriesMaterialApiPatch,
  type SeriesMaterialPatchInput,
} from "../model/series-material-patch";
...
const STATUS_TONE: Record<SeriesMaterialStatus, string> = {
  ...
  ACTIVE: "bg-sky-100 text-sky-900 border-sky-300 dark:bg-sky-950/40 dark:text-sky-200",
  IN_REVIEW:
...
function currentMaterialVersion(material: SeriesMaterial) {
  return (
    material.versions.find((version) => version.version === material.currentVersion) ??
    material.versions[0]
  );
}
...
export function SeriesMaterialsLibrary({
  ...
  const updateMaterial = (id: string, patch: SeriesMaterialPatchInput) => {
    updateMutation.mutate(
      {
        materialId: id,
        ...toSeriesMaterialApiPatch(patch),
      },
      {
        onError: (e) => toast.error(e instanceof Error ? e.message : "Unable to update material."),
      },
    );
  };
  ...
  const removeMaterial = (id: string) => {
    deleteMutation.mutate(id, {
      onError: (e) => toast.error(e instanceof Error ? e.message : "Unable to delete material."),
    });
  };
  ...
  const canEdit = !!user && (user.role === "editor" || user.role === "mangaka");
  ...
  const filtered = useMemo(() => {
    ...
    let list = materials.filter((m) => {
      ...
      if (!q) return true;
      const cur = currentMaterialVersion(m);
      return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => toast.info("Folder creation feature is coming soon.")}
...
function MaterialCard({
  ...
  const cur = currentMaterialVersion(material);
        ...
        <div className="mt-1 flex items-center gap-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="text-xs">
              {canEdit && material.status !== "APPROVED" ? (
      ...
      <div>
        ...
        <ul className="space-y-1.5">
          {material.versions.map((v) => {
            const isCurrent = v.version === material.currentVersion;
            return (
              <li
                key={v.id}
              ...
              >
                <div
                  className={`mt-0.5 grid size-5 place-items-center rounded-full ${isCurrent ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}
                >
                  {isCurrent ? (
