        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {visibleRows.map((row) => (
              <tr
                key={row.index}
                className={cn("border-t border-border/60", !row.valid && "bg-accent/5")}
              >
                ...
                <td className="px-2 py-1.5">
                  {row.valid ? (
                    <span className="text-[var(--role-editor)]">OK</span>
                  ) : (
                    <span className="text-accent" title={row.errors.join("; ")}>
                      {row.errors[0]}
                    </span>
                  )}
                </td>
