...
export type QueueColumn<T> = {
  ...
  /** Enables click-to-sort on this column's header. Requires sortKey/sortDirection/onSort on QueueTable. */
  sortable?: boolean;
};
...
export function QueueTable<T>({
  ...
  fixed?: boolean;
  empty?: ReactNode;
  /** Optional click-to-sort support. Set `sortable: true` on a column to enable it. */
  sortKey?: string | null;
                    ...
                    >
                      {col.sortable && onSort ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 select-none outline-none hover:opacity-80"
                        >
                          {col.header}
                          {active && sortDirection === "asc" ? (
