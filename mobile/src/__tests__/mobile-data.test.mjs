...
const boardPanelsSource = readFileSync(
  new URL("../screens/board-panels.tsx", import.meta.url),
  "utf8",
);
const editorPanelsSource = readFileSync(
  new URL("../screens/editor-panels.tsx", import.meta.url),
  "utf8",
);
const boardActionPanelsSource = readFileSync(
  new URL("../screens/board-action-panels.tsx", import.meta.url),
  "utf8",
);
const editorActionPanelsSource = readFileSync(
  new URL("../screens/editor-action-panels.tsx", import.meta.url),
  "utf8",
);
const proposalSummaryPanelSource = readFileSync(
...
test("legacy role-flow actions retain confirmation detail while Today remains Queue-first", () => {
  ...
  assert.doesNotMatch(editorHookSource, /editorApproveSubmission/);
  assert.doesNotMatch(editorHookSource, /requestEditorSubmissionRevision/);
  assert.match(editorHookSource, /reopenSelectedComment/);
  assert.match(
    editorActionPanelsSource,
    /Live endpoint: POST \/api\/editor\/series\/:seriesId\/start-review/,
  );
  assert.match(
    editorActionPanelsSource,
    /Live endpoint: POST \/api\/editor\/series\/:seriesId\/forward-to-board/,
  );
  assert.match(editorActionPanelsSource, /Live endpoint: POST \/api\/comments/);
  assert.match(editorActionPanelsSource, /Assistant submissions are reference/);
  assert.match(
    editorActionPanelsSource,
    /Backend validation, permissions, notification, and audit stay server-owned/,
  );
...
test("mobile screens expose reusable loading error and empty states", () => {
  ...
  assert.match(editorSource, /showBlockingOnly/);
  assert.match(editorSource, /showBlockersOnly/);
...
test("mobile rich detail previews preserve backend-owned workflow boundaries", () => {
  ...
  assert.match(
    boardPanelsSource,
    /Manual Board attention is required; cancellation is not automatic/,
  );
