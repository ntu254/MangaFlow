...
export function BoardHistoryScreen() {
  ...
  if (history.error && !history.data) return <WorkflowState kind="error" context="the governance decision ledger" error={history.error} onRetry={() => void history.refetch()} />
  ...
  return <>
    <MFHero role="board" title="Governance Decision Ledger" subtitle="Immutable Board record, separated by voting rounds and governance outcomes." />
    ...
    {entries.length ? sections.map((section) => <Fragment key={section.id}>
    ...
    </Fragment>) : <MFEmptyState title="No governance decisions recorded" subtitle="Finalized, tied, cancelled, and at-risk Board records will appear here." icon="shield-check" />}
  </>
}
