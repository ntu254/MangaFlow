...
function commentTone(status: CommentStatus, isBlocking: boolean): Tone {
  if (status === "RESOLVED") return "neutral";
  if (status === "REOPENED") return "warning";
  return isBlocking ? "danger" : "primary";
}
...
export const apiMobileWorkflowDataSource: MobileWorkflowDataSource = {
  ...
  finalizeBoardDecision: async (_seriesId, input) => {
    await apiMutate(`/voting-sessions/${input.sessionId}/close`, "board", {
      note: input.note,
    });
  },
  ...
  getEditorHome: async () => {
    ...
    const actions: MetricItem[] = [
      ...
      },
      {
        ...
        value: commentMetrics[0]?.value ?? "0",
        tone: "warning",
        icon: "message-circle",
        subtitle: "Loaded through API-ready boundary",
        actionLabel: "Resolve now",
      },
      {
        id: "blocked",
        label: "Check blocked chapters",
        value: readiness.overallPassed ? "0" : "1",
        tone: readiness.overallPassed ? "success" : "danger",
        icon: "lock",
        subtitle: "Backend readiness result",
    ...
    const queues: QueueItem[] = [
      ...
      {
        id: "comments",
        title: "Comments Boundary",
        subtitle: "Comments load per task once selected",
        value: "API",
        tone: "warning",
        icon: "message-square",
      },
      {
        ...
        value: readiness.overallPassed ? "0" : "1",
        tone: readiness.overallPassed ? "success" : "danger",
        icon: readiness.overallPassed ? "check-circle" : "alert-triangle",
      },
    ];
  ...
  getEditorSubmissions: async () => {
    ...
    return Promise.all(
      asArray<any>(items).map(async (item, index) => {
        ...
        // Editor approval lands on the chapter (READY_FOR_PUBLICATION), not on the
        // submission — submissions stay MANGAKA_APPROVED through editor review.
        const chapterStatus = text(chapterDetail?.status, "");
        return {
          id: itemId(item, `editor-submission-${index}`),
          seriesId:
            maybeId(taskDetail?.seriesId) ??
            maybeId(task?.seriesId) ??
            maybeId(chapterDetail?.seriesId),
          taskId,
          chapterId,
          ...
          assistantName: text(
            item?.submittedBy?.name ?? task?.assignedTo?.name,
            "Assigned assistant",
          ),
          mangakaNote: text(
            item?.reviewerNote ?? item?.resultText,
            "Mangaka approved; editor final check needed.",
          ),
  ...
  getEditorComments: async () => {
    ...
    const comments = asArray<any>(items).map((item, index) => {
      ...
      const tone = commentTone(canonicalStatus, isBlocking);
    ...
    return {
      metrics: [
        ...
        },
        {
          ...
          tone: reopened > 0 ? "warning" : "neutral",
          icon: "check-circle",
        },
        {
          id: "blocking",
  ...
  getBoardHome: async () => {
    ...
    const metrics: MetricItem[] = [
      ...
      },
      {
        id: "risk",
        label: "At-risk",
        value: numberText(atRiskReviews),
        tone: "danger",
        icon: "alert-triangle",
      },
    ];
    ...
    return {
      ...
      queues: [
        ...
        },
        {
          id: "ranking",
          title: "Ranking Cycle",
          subtitle: "Ranking API is available",
          value: "API",
          tone: "primary",
          icon: "bar-chart-2",
        },
  ...
  getBoardSeriesReviews: async () => {
    ...
    return asArray<any>(items)
      .filter((item) => item.seriesStatus === "BOARD_REVIEW")
      .map((item, index) => {
        const summary = item?.voteSummary ?? {};
        const eligible =
          typeof item?.eligibleBoardCount === "number"
            ? Math.max(item.eligibleBoardCount, 1)
        ...
        return {
          ...
          status:
            boardDecisionStatus(item?.decisionStatus) === "TIE_BREAK_REQUIRED"
              ? "Tie-break"
              : "Board Review",
          ...
          sessionId:
            typeof item?.activeVotingSessionId === "string"
              ? item.activeVotingSessionId
              : typeof item?.votingSessionId === "string"
                ? item.votingSessionId
                : typeof item?.sessionId === "string"
                  ? item.sessionId
                  : undefined,
          proposalVersionId:
            typeof item?.activeProposalVersionId === "string"
              ? item.activeProposalVersionId
              : typeof item?.proposalVersionId === "string"
  ...
  getBoardAtRiskCases: async () => {
    const items = await apiRequest<any[]>("/board/queue", "board");
    return asArray<any>(items)
      .filter((item) => item.seriesStatus === "AT_RISK")
      .map(
        (item, index) =>
          ({
