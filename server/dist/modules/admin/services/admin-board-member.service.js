...
export async function addAdminBoardMemberService(userId) {
    const user = await repository.getUserById(userId);
    if (!user)
        throw new AppError("User not found", 404);
    if (!user.isActive)
        throw new AppError("Only active users can be Board members", 403);
    if (user.role !== "BOARD")
        throw new AppError("Board member user must have BOARD role", 403);
    return repository.upsertBoardMember(userId);
}
export async function activateAdminBoardMemberService(userId) {
    const member = await repository.updateBoardMember(userId, { isActive: true });
    if (!member)
        throw new AppError("Board member not found", 404);
    return member;
}
export async function deactivateAdminBoardMemberService(userId) {
    const member = await repository.updateBoardMember(userId, { isActive: false, isChair: false });
    if (!member)
        throw new AppError("Board member not found", 404);
    return member;
}
export async function setAdminBoardChairService(userId) {
    const user = await repository.getUserById(userId);
    if (!user)
        throw new AppError("User not found", 404);
...
//# sourceMappingURL=admin-board-member.service.js.map