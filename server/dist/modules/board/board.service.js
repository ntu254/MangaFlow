...
async function assertBoardReviewSeries(seriesId, session) {
    ...
    if (series.status !== "BOARD_REVIEW")
        throw new AppError("Board vote requires Series in BOARD_REVIEW", 409);
    return series;
}
export async function castBoardVoteService(seriesId, userId, value) {
...
export async function finalizeBoardDecisionService(seriesId, userId) {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        await assertBoardReviewSeries(seriesId, session);
...
export async function tieBreakBoardDecisionService(seriesId, userId, value) {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        await assertBoardReviewSeries(seriesId, session);
