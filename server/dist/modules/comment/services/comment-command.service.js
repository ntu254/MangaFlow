...
import { assertEditorReopenTransition, assertEditorResolveTransition, assertMangakaVerifyTransition, assertMarkFixedTransition, } from "../guards/comment-transition.guard.js";
import { getCommentOrThrow } from "./comment.shared.js";
export async function markCommentFixedService(commentId, actor) {
...
export async function verifyCommentFixedService(commentId, actor) {
    const comment = await getCommentOrThrow(commentId);
...
export async function resolveCommentService(commentId, actor) {
    const comment = await getCommentOrThrow(commentId);
