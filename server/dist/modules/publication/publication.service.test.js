...
vi.mock("../chapter/chapter.service.js", async () => {
    const actual = await vi.importActual("../chapter/chapter.service.js");
...
describe("publication.service", () => {
    const actor = { userId: "editor1", role: "EDITOR" };
    ...
    it("creates a publication and mirrors scheduled date to chapter draft schedule", async () => {
        vi.mocked(repository.getPublicationChapter).mockResolvedValue(chapter);
        vi.mocked(repository.createPublicationRecord).mockResolvedValue({ ...publication, scheduledFor: new Date("2026-06-10T00:00:00.000Z") });
    ...
    it("schedules an existing publication", async () => {
        vi.mocked(repository.getPublicationById).mockResolvedValue(publication);
        vi.mocked(repository.updatePublicationSchedule).mockResolvedValue({ ...publication, scheduledFor: new Date("2026-06-11T00:00:00.000Z") });
    ...
    it("publishes only after readiness passes and transitions chapter", async () => {
        ...
        vi.mocked(repository.markPublicationPublished).mockResolvedValue({ ...publication, publishedAt: new Date() });
...
//# sourceMappingURL=publication.service.test.js.map