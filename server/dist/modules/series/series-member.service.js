...
// ---------------------------------------------------------------------------
export async function listSeriesMembersService(seriesId, actorId, actorRole) {
...
export async function updateSeriesMemberService(input) {
    ...
    if (member.status === "REMOVED")
        throw new AppError("Cannot update a removed member", 409);
...
// ---------------------------------------------------------------------------
export async function getEligibleAssistantsService(seriesId, actorId, actorRole) {
    const series = await Series.findById(seriesId);
    if (!series)
        throw new AppError("Series not found", 404);
    ...
    const activeMembers = await SeriesMember.find({
        ...
        role: "ASSISTANT",
        status: "ACTIVE",
    }).populate("userId", "name displayName email role isActive");
...
//# sourceMappingURL=series-member.service.js.map