...
export async function getSeriesDetailService(seriesId, userId, role) {
    try {
...
export async function createSeriesService(input) {
...
export async function submitSeriesService(seriesId, userId) {
...
export async function getSeriesSummaryService(seriesId, userId, role) {
    ...
    const fileById = new Map(data.files.map((file) => [String(file._id), file]));
    ...
    const manuscripts = data.manuscripts.map((manuscript) => {
        const file = manuscript.fileAssetId ? fileById.get(String(manuscript.fileAssetId)) : null;
        const uploader = manuscript.uploadedBy;
        return {
    ...
    const openComments = data.comments.filter((comment) => comment.status !== "RESOLVED_BY_EDITOR");
    const blockingComments = openComments.filter((comment) => comment.isBlocking);
    const totalPages = data.pages.length;
    const approvedPages = data.pages.filter((page) => page.status === "APPROVED").length;
    ...
    return {
        series,
        owner: data.owner ? {
            id: String(data.owner._id),
            name: data.owner.displayName || data.owner.name,
        ...
        taskSummary: {
            ...
            pending: data.tasks.filter((task) => !completedTaskStatuses.has(task.status)).length,
            completed: data.tasks.filter((task) => completedTaskStatuses.has(task.status)).length,
            pendingReviews: data.submissions.filter((submission) => ["SUBMITTED", "MANGAKA_APPROVED"].includes(submission.status)).length,
        },
        recentTasks: data.tasks.slice(0, 5).map((task) => ({
            id: String(task._id),
            title: task.title,
            status: task.status,
        ...
        recentComments: data.comments.slice(0, 5).map((comment) => ({
        ...
        boardReview: data.boardDecision ? {
            status: data.boardDecision.status,
            result: data.boardDecision.result,
            voteCount: data.boardVotes.length,
            updatedAt: data.boardDecision.updatedAt,
        } : null,
        publicationSummary: {
            isReady: chapters.length > 0 && readinessPercent === 100 && blockingComments.length === 0,
            scheduled: data.publications.filter((publication) => publication.scheduledFor && !publication.publishedAt).length,
            published: data.publications.filter((publication) => publication.publishedAt).length,
            blockers: [
        ...
        rankingSummary: data.ranking ? {
            period: data.ranking.period,
            voteCount: data.ranking.voteCount,
            readerScore: data.ranking.readerScore,
            finalScore: data.ranking.finalScore,
            status: data.ranking.status,
        } : null,
        payrollSummary: {
            totalEarnings: data.earnings.reduce((sum, earning) => sum + earning.finalPayment, 0),
            unpaid: data.earnings.filter((earning) => unpaidStatuses.has(earning.status)).reduce((sum, earning) => sum + earning.finalPayment, 0),
        },
        allowedActions: {
...
export async function updateSeriesService(input) {
    ...
    if (patch.synopsis !== undefined)
        patch.synopsis = patch.synopsis.trim();
    if (patch.logline !== undefined)
        patch.logline = patch.logline.trim();
    if (patch.premise !== undefined)
        patch.premise = patch.premise.trim();
    if (patch.characters !== undefined)
        patch.characters = patch.characters.trim();
    ...
    try {
        ...
        const message = String(error.message ?? "");
        if (message.includes("Only the owner Mangaka"))
            throw new AppError("Only the owner Mangaka can update this series", 403);
        if (message.includes("Series can only be edited"))
            throw new AppError("Series can only be edited while in DRAFT or REVISION_REQUESTED", 409);
        if (message.includes("Series not found"))
            throw new AppError("Series not found", 404);
