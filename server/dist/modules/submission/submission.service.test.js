...
import { createTaskSubmissionService, editorApproveSubmissionService, listReviewQueueSubmissionsService, mangakaApproveSubmissionService, rejectSubmissionService, requestSubmissionRevisionService, } from "./submission.service.js";
...
describe("submission review service", () => {
    ...
    it("lets the assigned Assistant create a new submitted version and moves task to SUBMITTED", async () => {
        vi.mocked(repository.getTaskForSubmission).mockResolvedValue(task);
        ...
        expect(repository.updateTaskStatusForSubmission).toHaveBeenCalledWith("task1", "SUBMITTED");
    });
    it("blocks Assistants from submitting someone else's task", async () => {
        ...
        await expect(createTaskSubmissionService({
            taskId: "task1",
            actor: { userId: "assistant1", role: "ASSISTANT" },
            resultText: "Nope",
    ...
    it("lets Mangaka approve submitted work and moves task to MANGAKA_APPROVED", async () => {
        ...
        vi.mocked(repository.getTaskForSubmission).mockResolvedValue(submittedTask);
        vi.mocked(SeriesMember.findOne).mockResolvedValue({
            isActive: true,
            role: "MANGAKA",
        });
        ...
        expect(repository.updateTaskStatusForSubmission).toHaveBeenCalledWith("task1", "MANGAKA_APPROVED");
    });
    it("blocks Editor final approval before Mangaka approval", async () => {
        ...
        await expect(editorApproveSubmissionService({
            submissionId: "submission1",
    ...
    it("lets Editor final-approve only after Mangaka approval and moves task to EDITOR_APPROVED", async () => {
        ...
        vi.mocked(repository.getTaskForSubmission).mockResolvedValue(mangakaApprovedTask);
        vi.mocked(SeriesMember.findOne).mockResolvedValue({
            isActive: true,
            role: "EDITOR",
        });
        ...
        expect(repository.updateTaskStatusForSubmission).toHaveBeenCalledWith("task1", "EDITOR_APPROVED");
    });
    it("lets Editor request revision after Mangaka approval", async () => {
        ...
        expect(repository.updateTaskStatusForSubmission).toHaveBeenCalledWith("task1", "REVISION_REQUESTED");
    });
    it("allows rejection only before Mangaka approval", async () => {
        ...
        await expect(rejectSubmissionService({
            submissionId: "submission1",
            actor: { userId: "mangaka1", role: "MANGAKA" },
    ...
    it("lists Mangaka review queue submissions for active Mangaka memberships", async () => {
        ...
        vi.mocked(repository.listReviewQueueSubmissions).mockResolvedValue([{ id: "submission1", status: "SUBMITTED" }]);
    ...
    it("lists Editor review queue submissions after Mangaka approval", async () => {
        ...
        vi.mocked(repository.listReviewQueueSubmissions).mockResolvedValue([{ id: "submission1", status: "MANGAKA_APPROVED" }]);
    ...
    it("blocks Assistants from review queue access", async () => {
        await expect(listReviewQueueSubmissionsService({ userId: "assistant1", role: "ASSISTANT" })).rejects.toThrow("Review queue access denied");
    });
});
