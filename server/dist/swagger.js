...
const doc = {
    info: {
        version: "1.0.0",
        title: "MangaFlow API",
        description: "MangaFlow internal production workflow API. Response envelope is `{ success, message, data }`; errors use `{ success, message, errors? }`.",
    },
    host: "localhost:3001",
    basePath: "/",
    schemes: ["http"],
...
function tagFor(pathName) {
    ...
    if (pathName.startsWith("/api/series"))
        return "Series";
    if (pathName.startsWith("/api/manuscripts"))
        return "Manuscripts";
    ...
    if (pathName.startsWith("/api/files"))
        return "Files";
    if (pathName.startsWith("/api/tasks/types") || pathName.startsWith("/api/task-types"))
        return "Task Types";
...
function successDescription(method) {
    if (method === "post")
        return "Created or action completed";
    if (method === "patch")
        return "Updated";
    if (method === "delete")
        return "Deleted";
...
function applyRequestBody(op, method, pathName) {
