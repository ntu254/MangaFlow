...
export async function updateChapterStatus(
  ...
  const actor = { userId: req.user!.userId, role: req.user!.role };
  const chapter = await updateChapterStatusService(
...
export async function sendChapterToEditor(
  req: Request,
  res: Response,
  _next: NextFunction,
): Promise<void> {
  const actor = { userId: req.user!.userId, role: req.user!.role };
  const result = await sendChapterToEditorService(
    String(req.params.chapterId),
    actor,
  );
  res.json({
