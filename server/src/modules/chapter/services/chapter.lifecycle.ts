...
export async function updateChapterStatusService(
  chapterId: string,
  status: string,
  actor: AccessActor,
) {
  ...
  if (status === "IN_PRODUCTION") {
    if (chapter.status !== "DRAFT") {
      throw new AppError(
        "Only DRAFT chapters can be transitioned to IN_PRODUCTION",
        409,
      );
    }
    ...
    if (!["DRAFT", "IN_PRODUCTION"].includes(chapter.status)) {
      throw new AppError(
        "Only DRAFT or IN_PRODUCTION chapters can be archived",
        409,
      );
    }
  } else {
    throw new AppError(
      "Chapter lifecycle transitions are managed by page upload, readiness, and publication workflows",
      409,
    );
  }
...
export async function sendChapterToEditorService(
  chapterId: string,
  actor: AccessActor,
) {
  const trimmed = chapterId.trim();
  ...
  if (chapter.status !== "IN_PRODUCTION") {
    throw new AppError(
      "Only chapters in production can be sent to Editor review.",
      409,
    );
  }

  const [pages, tasks] = await Promise.all([
    Page.find({ chapterId: trimmed, deletedAt: { $exists: false } }).lean(),
    Task.find({ chapterId: trimmed }).lean(),
  ]);

  if (pages.length === 0) {
    throw new AppError(
      "Upload at least one page before sending this chapter to Editor.",
      400,
    );
  }

  const blockedPage = pages.find((page) =>
    [
      "PENDING",
      "UPLOADING",
      "PROCESSING",
  ...
  if (blockedPage) {
    throw new AppError(
      "Resolve page upload or processing blockers before sending this chapter to Editor.",
      400,
    );
  }

  const handoffReadyStatuses = ["MANGAKA_APPROVED", "EDITOR_APPROVED"];
  const unfinishedTask = tasks.find(
    (task) => !handoffReadyStatuses.includes(task.status),
  );
  if (unfinishedTask) {
    throw new AppError(
      "Mangaka must approve all Assistant submissions before sending this chapter to Editor.",
      400,
    );
  }

  await Promise.all(tasks.map((task) => syncFinishedTaskTarget(task)));

  return {
    chapter,
    pages: pages.length,
    tasks: tasks.length,
    pendingEditorReviews: tasks.filter(
      (task) => task.status === "MANGAKA_APPROVED",
    ).length,
  };
}

export async function deleteChapterService(
  chapterId: string,
  actor: AccessActor,
) {
  const trimmed = chapterId.trim();
  ...
  if (
    ...
    throw new AppError(
      `Cannot delete a chapter that is ${chapter.status}. Archive it instead.`,
      400,
    );
  }
  ...
  const activeSubmissionsCount = await Submission.countDocuments({
    chapterId: trimmed,
    status: {
      $in: ["DRAFT", "SUBMITTED", "REVISION_REQUESTED", "MANGAKA_APPROVED"],
    },
  });

  if (activeTasksCount > 0 || activeSubmissionsCount > 0) {
    throw new AppError(
      "Cannot delete chapter with active tasks or submissions. Cancel or resolve tasks first.",
      400,
    );
  }
...
export async function cancelChapterService(
  chapterId: string,
  actor: AccessActor,
) {
  const trimmed = chapterId.trim();
  ...
  if (!["DRAFT", "IN_PRODUCTION"].includes(chapter.status)) {
    throw new AppError(
      "Only DRAFT or IN_PRODUCTION chapters can be cancelled",
      400,
    );
  }
