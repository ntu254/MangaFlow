...
export async function normalizeCommentScope(input: CreateCommentServiceInput): Promise<NormalizedCommentScope> {
  ...
  if (input.taskId) {
    ...
    if (String(task.seriesId) !== input.seriesId) throw new AppError("Task does not belong to the specified series", 400)
  ...
  if (scope.regionId) {
    if (!scope.pageId) throw new AppError("Page ID is required when commenting on a region", 400)
