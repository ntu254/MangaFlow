...
export async function validateTaskCreationScope(input: {
  ...
  if (String(chapter.seriesId) !== input.seriesId) throw new AppError("Chapter does not belong to the specified series", 400)

  const taskType = await TaskType.findById(input.taskTypeId)
  if (!taskType) throw new AppError("Task type not found", 404)
  if (!taskType.isActive) throw new AppError("Task type is not active", 409)
  ...
  if (input.regionId) {
    if (!input.pageId) throw new AppError("Page ID is required when assigning a region task", 400)
