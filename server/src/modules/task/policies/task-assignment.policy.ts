...
export async function assertTaskAssignmentAllowed(input: { seriesId: string; assignedTo: string; assignedBy: string }): Promise<void> {
  ...
  if (!assigneeActive) throw new AppError("Assigned Assistant is not active in this series", 403)
  ...
  if (!assignerActive) throw new AppError("Assigner is not active in this series", 403)
}
