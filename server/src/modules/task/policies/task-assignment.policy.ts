...
export async function assertTaskAssignmentAllowed(input: { seriesId: string; assignedTo: string; assignedBy: string }): Promise<void> {
  ...
  if (!assigneeMembership) throw new AppError("Assigned user is not a member of this series", 403)
  if (assigneeMembership.role !== "ASSISTANT") throw new AppError("Only Assistants can be assigned tasks", 403)

  const activeAssigneeMember = await findActiveSeriesMember(input.seriesId, input.assignedTo)
  if (!activeAssigneeMember) throw new AppError("Assigned Assistant is not active in this series", 403)
  if (activeAssigneeMember.accessScope !== "TASK_ONLY") throw new AppError("Assigned Assistant must use TASK_ONLY access scope", 403)

  const activeAssignerMember = await findActiveSeriesMember(input.seriesId, input.assignedBy)
  if (!activeAssignerMember) throw new AppError("Assigner is not active in this series", 403)
  if (!["MANGAKA", "EDITOR"].includes(activeAssignerMember.role)) throw new AppError("Only Mangaka or Editor can assign tasks", 403)
}
