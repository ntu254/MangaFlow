...
export type CommentStats = { blocking: number; open: number; fixed: number; total: number };
...
export function commentTone(comment: StudioComment): CommentTone {
  ...
  if (comment.status === "FIXED") return "amber";
  return "blue";
}
...
export function statsForComments(comments: StudioComment[]): CommentStats {
  ...
  for (const c of comments) {
    ...
    else if (c.status === "FIXED") fixed += 1;
    else open += 1;
  }
  return { blocking, open, fixed, total: comments.length };
}

export function pageBadge(stats: CommentStats): PageBadge {
  if (stats.blocking > 0) return { label: "Blocking", tone: "rose" };
  if (stats.fixed > 0) return { label: "Fixed", tone: "amber" };
  if (stats.open > 0)
